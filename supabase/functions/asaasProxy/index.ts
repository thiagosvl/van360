...
const WEBHOOK_AUTH_TOKEN = Deno.env.get("ASAAS_WEBHOOK_TOKEN");
...
serve(async (req) => {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return new Response(JSON.stringify({ error: "Autenticação requerida (JWT ausente)." }), { status: 401 });
    }
    ...
    if (authError || !userData?.user) {
        return new Response(JSON.stringify({ error: "Token JWT inválido ou expirado." }), { status: 401 });
    }
    ...
    if (dbError || !userDataDB?.asaas_subaccount_api_key) {
        ...
        return new Response(JSON.stringify({ error: "Chave Asaas do usuário não encontrada (Falha na ligação DB/Auth)." }), { status: 403 });
    }
    ...
    try {
        ...
        if (path.startsWith("/webhooks") && req.method === "POST" && requestBody) {
            if (WEBHOOK_AUTH_TOKEN) {
                requestBody.authToken = WEBHOOK_AUTH_TOKEN;
            } else {
        ...
        return new Response(
            JSON.stringify({ error: "Erro interno do servidor ao comunicar com Asaas." }),
