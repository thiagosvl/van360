  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className={`${cardStyle} border-l-4 border-l-indigo-500`}>
            <CardContent className="p-5 flex flex-col justify-center">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="p-1.5 bg-indigo-50 text-indigo-600 rounded-md"><Layers className="w-4 h-4" /></div>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <Card className={`${cardStyle} border-l-4 border-l-blue-500`}>
            <CardContent className="p-5 flex flex-col justify-center">
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">ARPU Mensal</span>
                <div className="p-1.5 bg-blue-50 text-blue-600 rounded-md"><BadgeDollarSign className="w-4 h-4" /></div>
              </div>
              <span className="text-2xl lg:text-3xl font-black text-blue-700 tracking-tight">{formatCurrency(arpu)}</span>
              <span className="text-xs text-slate-500 mt-2 font-medium">Ticket médio geral por condutor</span>
            </CardContent>
          </Card>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardContent className="p-6 space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-white/60 p-4 rounded-xl border border-slate-200/50 shadow-sm">
                ...
                <div className="text-2xl font-black text-emerald-600">{formatCurrency(lucroBase)}</div>
              ...
              <div className="bg-white/60 p-4 rounded-xl border border-slate-200/50 shadow-sm">
                ...
                <div className="text-2xl font-black text-blue-600">{formatCurrency(lucroAddon)}</div>
                <div className="text-xs font-medium text-slate-400 mt-1">
            ...
            <div className="space-y-2">
              ...
              <div className="h-3 w-full rounded-full overflow-hidden flex shadow-inner">
                <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${bBarPct}%` }} />
                <div className="h-full bg-blue-500 transition-all duration-1000" style={{ width: `${100 - bBarPct}%` }} />
              </div>
              <div className="flex gap-4 text-xs font-medium text-slate-600 mt-3 justify-center">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-[3px] bg-emerald-500 shadow-sm" />
                  <span>Assinatura ({bBarPct.toFixed(1)}%)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-[3px] bg-blue-500 shadow-sm" />
        ...
        <Card className={cardStyle}>
          <CardHeader className="pb-3 border-b border-slate-100/50">
            <CardTitle className="text-sm font-bold text-slate-800 uppercase tracking-wider">DRE Simplificado Mensal</CardTitle>
          </CardHeader>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
        <Card className={`${cardStyle} bg-gradient-to-r from-blue-900 to-indigo-900 border-none overflow-hidden relative shadow-xl`}>
          {/* Decorative background circles */}
          <div className="absolute -right-20 -top-20 w-64 h-64 bg-blue-500/20 rounded-full blur-3xl"></div>
          <div className="absolute right-40 -bottom-20 w-64 h-64 bg-indigo-500/20 rounded-full blur-3xl"></div>
          
          <CardContent className="p-8 relative z-10">
            <div className="flex flex-col md:flex-row gap-8 justify-between items-center">
              <div className="space-y-2 flex-1">
                <BadgeDollarSign className="w-8 h-8 text-blue-300 mb-4" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-[2] w-full">
                <div className="bg-white/10 backdrop-blur-md p-5 rounded-xl border border-white/10">
                  <p className="text-xs font-medium text-blue-200 uppercase tracking-wider mb-2">Lucro Projetado (Atual)</p>
                ...
                <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/20 backdrop-blur-md p-5 rounded-xl border border-emerald-400/30 shadow-[0_0_30px_-5px_rgba(16,185,129,0.3)]">
                  <p className="text-xs font-medium text-emerald-200 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <TrendingUp className="w-4 h-4" /> Lucro c/ Churn de {churnSensitivity.improvedChurn}%
                  </p>
                  <p className="text-3xl font-black text-emerald-400 mb-1">{formatCurrency(churnSensitivity.improvedProfit)}</p>
                  <p className="text-xs font-bold text-emerald-300">Uplift de +{formatCurrency(churnSensitivity.difference)} no bolso</p>
                </div>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex gap-6 items-center">
              <div className="space-y-1.5 w-40">
                <div className="flex justify-between text-xs font-semibold text-slate-600">
                  ...
                  <span className="text-indigo-600">+{consolidatedState.growPct}%/mês</span>
                </div>
              ...
              <div className="space-y-1.5 w-40">
                <div className="flex justify-between text-xs font-semibold text-slate-600">
                  ...
                  <span className="text-indigo-600">{consolidatedState.addonPct}%</span>
                </div>
          ...
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={consolidatedChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="base" name="Lucro Assinatura" stackId="1" stroke="#10b981" strokeWidth={3} fill="url(#colorBase)" activeDot={{ r: 6, strokeWidth: 0 }} />
                <Area type="monotone" dataKey="addon" name="Lucro Add-on" stackId="1" stroke="#3b82f6" strokeWidth={3} fill="url(#colorAddon)" activeDot={{ r: 6, strokeWidth: 0 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
        ...
        <Card className={`${cardStyle} border-l-4 border-l-amber-500 overflow-hidden relative`}>
          ...
          <CardContent className="p-6 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                  <span className={`w-2 h-2 rounded-full ${bBarPct < 100 ? 'bg-blue-500' : 'bg-slate-300'}`}></span>
