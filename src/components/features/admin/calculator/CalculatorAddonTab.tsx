      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className={`${cardStyle} border-l-4 border-l-blue-500`}>
            <CardContent className="p-5 flex flex-col justify-center text-left">
              ...
              <span className="text-xs text-slate-400 mt-2 font-medium">Nas rotas ativas</span>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className={`${cardStyle} border-l-4 border-l-blue-500`}>
            <CardContent className="p-5 flex flex-col justify-center text-left">
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Receita Bruta (Add-on)</span>
              ...
              <span className="text-xs text-slate-400 mt-2 font-medium">Faturamento total</span>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card className={`${cardStyle} border-l-4 border-l-orange-500`}>
            <CardContent className="p-5 flex flex-col justify-center text-left">
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Custo Split Pag.</span>
                <div className="p-1.5 bg-orange-500/10 text-orange-400 rounded-md border border-orange-500/20"><Percent className="w-4 h-4" /></div>
              </div>
              <span className="text-2xl lg:text-3xl font-black text-white tracking-tight">{formatCurrency(splitCust)}</span>
              <span className="text-xs text-slate-400 mt-2 font-medium">Repassado ao Gateway</span>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <Card className={`${cardStyle} border-l-4 ${isProfit ? 'border-l-emerald-500' : 'border-l-red-500'}`}>
            <CardContent className="p-5 flex flex-col justify-center text-left">
              ...
              <span className="text-xs text-slate-400 mt-2 font-medium">Margem {margem.toFixed(1)}%</span>
            </CardContent>
          </Card>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="font-medium text-slate-300">Condutores c/ Add-on Ativo</span>
                    <span className="font-bold text-blue-400 bg-blue-500/10 px-2.5 py-0.5 rounded-md border border-blue-500/20">{addonState.nc}</span>
                  </div>
          ...
          <Card className={cardStyle}>
            <CardHeader className="pb-4 border-b border-slate-800/80 text-left">
              <div className="flex items-center gap-2">
                <div className="p-2 bg-blue-500/10 text-blue-400 rounded-lg border border-blue-500/20"><DollarSign className="w-5 h-5" /></div>
                <div>
                  <CardTitle className="text-lg font-bold text-white">Split & Impostos</CardTitle>
                  <CardDescription className="text-slate-400">Configurações de custo e churn.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-6 space-y-6 text-left">
                <div className="space-y-2">
                  <label className="text-[10px] font-semibold text-slate-400 uppercase">Custo do Gateway (Split)</label>
                  <Select value={addonState.splitTipo} onValueChange={(val) => updateAddon('splitTipo', val)}>
                    <SelectTrigger className="w-full h-9 bg-slate-900 border-slate-800 text-slate-100 text-sm">
                      <SelectValue />
                    </SelectTrigger>
                ...
                {addonState.splitTipo === 'fixo' ? (
                  <div className="flex justify-between items-center gap-4">
                    <span className="text-sm text-slate-300">Valor fixo (R$)</span>
                    <AdminMoneyInput className="w-[120px] bg-slate-900 border-slate-800 text-slate-100 text-sm" value={addonState.splitFixo} onChange={(v) => updateAddon('splitFixo', v)} />
                  </div>
                ) : (
                  <div className="flex justify-between items-center gap-4">
                    <span className="text-sm text-slate-300">Percentual (%)</span>
                    <Input type="number" className="w-[100px] bg-slate-900 border-slate-800 text-slate-100 text-sm" value={addonState.splitPct} onChange={e => updateAddon('splitPct', parseFloat(e.target.value) || 0)} />
                  </div>
                )}
                ...
                <div className="flex justify-between items-center gap-4">
                  <div className="flex flex-col">
                    <span className="text-sm text-slate-300">Imposto receita (%)</span>
                    <span className="text-[10px] text-slate-400">incide pós-split</span>
                  </div>
                  <Input type="number" className="w-[100px] bg-slate-900 border-slate-800 text-slate-100 text-sm" value={addonState.tImp} onChange={e => updateAddon('tImp', parseFloat(e.target.value) || 0)} />
                </div>
                
                <div className="flex justify-between items-center gap-4">
                  <div className="flex flex-col">
                    <span className="text-sm text-slate-300">Churn do Add-on (%)</span>
                    <span className="text-[10px] text-slate-400">taxa de cancelamento</span>
                  </div>
                  <Input type="number" className="w-[100px] bg-slate-900 border-slate-800 text-slate-100 text-sm" value={addonState.churn} onChange={e => updateAddon('churn', parseFloat(e.target.value) || 0)} />
                </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card className={cardStyle}>
            <CardHeader className="pb-3 border-b border-slate-800/80 text-left">
              <CardTitle className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-emerald-400" /> Economics Mensais (Global)
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm p-5 bg-slate-900/60 rounded-b-xl text-left">
              <div className="flex justify-between py-1.5 font-medium">
                <span className="text-slate-300">Receita Bruta Add-on</span>
                <span className="text-white font-bold">{formatCurrency(brutaTotal)}</span>
              </div>
              ...
              <div className="flex justify-between items-center py-2">
                <span className="font-black text-base text-slate-200">Lucro Líquido Add-on</span>
                <span className={`font-black text-lg ${lucro >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                  {formatCurrency(lucro)}
                </span>
              </div>
            </CardContent>
          ...
          <Card className={`${cardStyle} bg-gradient-to-br from-blue-900/20 to-slate-900/80 border-blue-500/30`}>
            <CardHeader className="pb-3 border-b border-slate-800 text-left">
              <CardTitle className="text-sm font-bold text-blue-400 uppercase tracking-wider">
                Unit Economics (1 Condutor)
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm p-5 text-left">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-slate-900 p-3 rounded-lg border border-slate-800">
                  <span className="block text-[10px] font-semibold text-slate-400 uppercase">Receita Gerada</span>
                  <span className="block text-lg font-bold text-white">{formatCurrency(rb1)}</span>
                </div>
                <div className="bg-slate-900 p-3 rounded-lg border border-slate-800">
                  <span className="block text-[10px] font-semibold text-slate-400 uppercase">Lucro Incremental</span>
                  <span className="block text-lg font-bold text-emerald-400">{formatCurrency(ll1)}</span>
                </div>
              </div>
