  ...
  if (!logs || logs.length === 0) {
    return (
      <div className="text-center py-16 space-y-3">
        <Terminal className="h-12 w-12 mx-auto text-slate-600" />
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {logs.map((log) => {
              ...
              return (
                <tr key={log.id} className="border-b border-slate-800/40 hover:bg-slate-800/50 transition-colors">
                  <td className="py-4 text-xs font-semibold text-slate-300">
                    {dateFormatted}
                  </td>
                  <td className="py-4">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-slate-800 text-slate-300 border border-slate-700/60">
                      {actionLabel}
                    </span>
                  </td>
                  <td className="py-4 hidden sm:table-cell">
                    {log.usuarios ? (
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[10px] font-medium text-slate-400">{log.usuarios.telefone}</span>
                      </div>
                    ) : (
                      <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">—</span>
                    )}
                  </td>
                  <td className="py-4 text-xs font-bold text-slate-400 uppercase tracking-wide hidden sm:table-cell">
                    {log.entidade_tipo}
                  </td>
                  <td className="py-4">
                    <div className="text-xs font-medium text-slate-300 max-w-[360px] whitespace-normal break-words" title={log.descricao}>
                      {log.descricao}
                    </div>
                  </td>
                  <td className="py-4 hidden md:table-cell">
                    <code className="text-[10px] bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800 font-mono text-slate-400">
                      {log.ip_address || "—"}
                    </code>
                  </td>
                  <td className="py-4 text-right">
                    {log.meta && Object.keys(log.meta).length > 0 ? (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 rounded-xl text-blue-400 hover:bg-blue-500/10 hover:text-blue-300 px-2 flex items-center gap-1.5 ml-auto"
                      ...
                      <span className="text-xs text-slate-500 pr-4">—</span>
                    )}
                  </td>
                </tr>
              );
            ...
            <div key={log.id} className="p-4 bg-slate-900/80 rounded-2xl border border-slate-800 space-y-3 text-left">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-[10px]">
                  ...
                  {log.ip_address && (
                    <code className="text-[9px] bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800 font-mono text-slate-400">
                      {log.ip_address}
                    </code>
                  )}
                </div>
                <p className="text-xs font-medium text-slate-300 leading-relaxed break-words break-all">
                  {log.descricao}
                </p>
              </div>
      ...
      {selectedLog && (
        <BaseDialog open={!!selectedLog} onOpenChange={() => setSelectedLog(null)}>
          <BaseDialog.Header
            ...
            icon={<Terminal className="w-5 h-5 text-blue-400 bg-blue-500/10 rounded-full p-0.5" />}
            ...
            <div className="space-y-4 py-2 text-left">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-900/90 p-4 rounded-xl border border-slate-800">
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-200 mt-0.5 uppercase">{selectedLog.acao.replace(/_/g, " ")}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Entidade ID</p>
                  <p className="text-xs font-mono text-slate-300 mt-0.5 break-all" title={selectedLog.entidade_id}>
              ...
              <div className="space-y-1.5">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Dados Completos (JSON)</p>
                <pre className="bg-slate-950 text-slate-200 border border-slate-800 p-4 rounded-2xl text-xs overflow-x-auto font-mono max-h-[320px] scrollbar-thin select-all">
          ...
          <BaseDialog.Footer>
            <Button
              ...
              className="w-full h-11 rounded-xl bg-blue-600 text-xs font-bold uppercase tracking-wider shadow-lg shadow-blue-600/30 hover:bg-blue-500 text-white"
            >
              Copiar JSON
            </Button>
          </BaseDialog.Footer>
