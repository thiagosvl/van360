...
export default function PixKeyDialog({
  ...
  const renderStatusBanner = () => {
    ...
    // 2. Banner de Alteração (Aparece ao editar uma chave que já era válida)
    if (isValid && overrideStatus) {
      return (
        <div className="mb-6 bg-amber-50 border border-amber-100 rounded-xl p-4 flex gap-3 items-start">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-semibold text-amber-900">Alterando Chave PIX</h4>
    ...
    // 3. Banner Informativo Padrão (Fallback para quando está cadastrando pela primeira vez ou corrigindo)
    return (
  ...
  const renderFooter = () => {
    ...
    return (
      <div className="p-4 bg-gray-50 border-t border-gray-100 shrink-0 flex gap-3">
        <Button 
          type="submit" 
          disabled={isSubmitting || showPending}
          onClick={form.handleSubmit(handleSubmit)}
          className={cn(
            "flex-1 h-12 rounded-xl font-bold shadow-lg transition-all active:scale-95",
            showPending 
              ? "bg-blue-100 text-blue-600 shadow-none hover:bg-blue-100 cursor-default" 
