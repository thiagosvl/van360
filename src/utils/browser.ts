...
/**
 * Copia texto para a área de transferência com suporte universal
 * (funciona tanto em HTTPS/localhost quanto em HTTP por IP local via fallback).
 */
export async function copyToClipboard(text: string): Promise<boolean> {
  ...
  if (typeof navigator !== "undefined" && navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
    try {
      ...
      console.warn("Falha no navigator.clipboard.writeText, utilizando fallback:", err);
    }
  }
  ...
  try {
    ...
    console.error("Falha ao copiar texto via fallback:", err);
    return false;
  }
}