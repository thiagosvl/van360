    ...
    try {
      if (escola.ativo) {
        ...
        if (passageiros && passageiros.length > 0) {
          toast({
            title: "Aviso",
            description: "Não é possível desativar escola com passageiros vinculados",
            variant: "destructive",
          });
          return;
        }
  ...
  const handleDelete = async (escola: Escola) => {
    ...
    try {
      ...
      if (passageiros && passageiros.length > 0) {
        toast({
          title: "Erro",
          description: "Não é possível excluir escola com passageiros vinculados",
          variant: "destructive",
        });
        return;
      }
    ...
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-4xl mx-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex gap-2 pt-4">
                  <Button type="submit" disabled={loading} className="flex-1">
                    {loading ? "Salvando..." : editingEscola ? "Atualizar" : "Cadastrar"}
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
        ...
        <div className="grid gap-4">
          {escolas.map((escola) => (
            <Card key={escola.id} className={escola.ativo ? "" : "opacity-50"}>
              <CardHeader>
                <CardTitle className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {escola.ativo ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </Button>
                    {!escola.ativo && (
                      <Button
                        size="sm"
              ...
              <CardContent>
                <div className="text-sm text-muted-foreground space-y-1">
                  ...
                  {escola.cidade && escola.estado && (
                    <div>{escola.cidade} - {escola.estado}</div>
                  )}
