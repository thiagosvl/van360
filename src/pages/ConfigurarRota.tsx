...
import { RouteNodeType, RouteSentido, RouteExecutionStatus } from "@/types/route";
...
export default function ConfigurarRota() {
  ...
  const { setPageTitle, openConfirmationDialog, closeConfirmationDialog, openRouteFormDialog } = useLayout();

  useEffect(() => {
    setPageTitle(isEditing ? "Editar Rota" : "Configurar Rota");
  }, [setPageTitle, isEditing]);

  // Realtime Sync para Tela de Edição de Rota (Redireciona se a rota for iniciada ou excluída por outro dispositivo)
  useEffect(() => {
    if (!id) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (payloadData?.rotaId === id && payloadData?.status === RouteExecutionStatus.INICIADA && payloadData?.execucaoId) {
      ...
      .on(
        ...
        (payload: any) => {
          const payloadData = payload?.payload || payload;
          if (payloadData?.rotaId === id && payloadData?.action === "delete") {
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: `rota_id=eq.${id}`
        },
        (payload: any) => {
          if (payload.new?.status === RouteExecutionStatus.INICIADA && payload.new?.id) {
            toast.info("Esta rota foi iniciada.");
            queryClient.invalidateQueries({ queryKey: ["routes"] });
            navigate(ROUTES.PRIVATE.MOTORISTA.ROUTE_EXECUTE.replace(":id", payload.new.id), { replace: true });
          }
        }
      )
      .on(
  ...
  return (
    <PullToRefreshWrapper onRefresh={async () => { }}>
        ...
        <div className="space-y-5 mt-1">
          ...
          <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm flex items-center justify-between gap-4 transition-all">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              {isEditing && id && can("rotas.excluir") && (
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  ...
                  {itinerario.map((item, index) => {
                    ...
                    return (
                      <ItineraryNodeCard
                        ...
                        desces={desces}
                        subes={subes}
                        onToggleSentido={handleToggleSentido}
                        onMove={handleMove}
                        onRemove={handleRemove}
                        onInsertIntermediary={openModalParadaIntermediaria}
                        onOpenReordenarSheet={(item) => setReordenarSheetTargetItem(item)}
                      />
                    );
                  })}
      ...
      <ReordenarParadaSheet
        isOpen={reordenarSheetTargetItem !== null}
        onClose={() => setReordenarSheetTargetItem(null)}
        paradaTarget={reordenarSheetTargetItem as any}
        totalPendentes={itinerario as any}
        paradasConcluidas={[]}
        execucaoTipo=""
        validarMovimentoPermitido={validarMovimentoPermitido}
        onConfirmReordenação={(novasParadas) => setItinerario(novasParadas as any)}
      />
